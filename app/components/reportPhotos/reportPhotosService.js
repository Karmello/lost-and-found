var reportPhotosService = function($rootScope, $q, aws3Service, MySrcAction, ReportsRest, Restangular, URLS, myModalService) {
    ...
    uploadPhotos: function(actionId, scope, inputData, src) {
      ...
      scope.srcAction.validate(actionId, inputData).then(function(res) {
        ...
        if (res.success) {
          ...
          }, function(res) { myModalService.showCustom('tryAgainLaterModal'); });
...
reportPhotosService.$inject = ['$rootScope', '$q', 'aws3Service', 'MySrcAction', 'ReportsRest', 'Restangular', 'URLS', 'myModalService'];
