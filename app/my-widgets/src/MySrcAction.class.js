  ...
  MySrcAction.prototype.displayModalMessage = function(msgId, acceptCb) {
    ...
    switch (msgId) {
      ...
      case 'confirmDeletion':
        settings = { title: $rootScope.hardData.labels[28], message: $rootScope.hardData.warnings[0], acceptCb: acceptCb };
        $rootScope.$broadcast('confirmModalShow', settings);
        break;

      case 'MAX_FILES_UPLOADED':
        settings = { title: username, message: $rootScope.hardData.rejections[4] };
        $rootScope.$broadcast('infoModalShow', settings);
        break;
