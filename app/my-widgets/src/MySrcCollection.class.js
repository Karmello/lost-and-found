var MySrcCollection = function($rootScope, $q, MyCollectionSelector, MySrc, MyLoader, myModalService) {
  ...
  MySrcCollection.prototype.removeFromSet = function(args, cb) {
    ...
    if (args.collection.length === 0) {
      ...
      $rootScope.$broadcast('confirmModalShow', {
        title: $rootScope.hardData.labels[28] + ' (' + args.collection.length + ')',
        message: $rootScope.hardData.warnings[0],
        acceptCb: function() {
...
MySrcCollection.$inject = ['$rootScope', '$q', 'MyCollectionSelector', 'MySrc', 'MyLoader', 'myModalService'];
