...
module.exports = function(req, res, next) {
    ...
    r.User.findOne({ email: req.decoded._doc.email }, '-email -username', function(err, user) {

        if (!err && user) {

            user.password = Math.random().toString(36).slice(-8);

            var mail = r.modules.mailModule.create('new_pass', req.session.language, req.decoded._doc.email, {
