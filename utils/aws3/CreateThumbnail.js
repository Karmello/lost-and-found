...
exports.handler = function(event, context, callback) {
    ...
    if (FILE_TYPES.indexOf(imageType) == -1) {
    ...
    async.waterfall([function download(next) {
        ...
        gm(response.Body).size(function(err, size) {
            ...
            this.resize(width, height).toBuffer(imageType, function(err, buffer) {
                if (err) { next(err); } else { next(null, response.ContentType, buffer); }
            });
        });
        ...
        s3.putObject({ Bucket: dstBucket, Key: dstKey, ACL: 'public-read', Body: data, ContentType: contentType }, next);
